...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// Wordmark / FishLogo paint with currentColor, inherited from
		// --dsw-alias-label-primary (near-black / near-white). Brand-token
		// overrides never reach them — that is why the toggle can be On
		// while the whales stay black.
		const CSS = [
		...
		const zh = {
			"title": "蓝鲸",
			"hint": "用 DeepSeek 官方蓝 #4D6BFE 替换黑鲸主色。亮色 / 深色跟随系统外观设置。",
			"open": "打开",
			"close": "关闭",
			"statusOn": "已开启"
		};
		...
		const en = {
			"title": "Blue Whale",
			"hint": "Replace the black-whale brand with official DeepSeek blue #4D6BFE. Light and dark follow the built-in appearance setting.",
			"open": "Turn on",
			"close": "Turn off",
			"statusOn": "On"
		};
