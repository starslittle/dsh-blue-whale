...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// DeepSeek Chat brand blue (chat.deepseek.com / public brand).
		const BRAND = "#4D6BFE";
		// DSH --dsw-static-deepseek-* scale (in-repo palette).
		...
		const CSS = [
			...
			"body[data-dsh-blue-whale] .hHd-Xa_brand,",
			"body[data-dsh-blue-whale] .hHd-Xa_brand svg,",
			"body[data-dsh-blue-whale] svg:has(#dsh-wordmark-whale-clip) {",
			"  color: #4D6BFE !important;",
			"}",
			"body[data-dsh-blue-whale] svg:has(#dsh-wordmark-whale-clip) path[fill=\"currentColor\"] {",
			...
			"  align-items: center;",
			"}",
			".dsh-blue-whale-btn-close {",
			"  background: var(--dsw-alias-bg-module-platform);",
			"  color: var(--dsw-alias-label-primary);",
			"}",
			".dsh-blue-whale-btn-close:hover {",
			"  background: var(--dsw-alias-interactive-bg-hover);",
			"}",
			".dsh-blue-whale-btn-open {",
			"  background: #4D6BFE;",
			"  color: #ffffff;",
			"}"
		].join("\n");
		...
		const zh = {
			"title": "蓝鲸",
			"hint": "侧栏鲸标、欢迎页鲸标、浏览器标签页和品牌色一并换成 Chat 蓝 #4D6BFE。亮色 / 深色跟随系统外观。",
			"open": "打开",
			"close": "关闭",
			"statusOn": "已开启"
		};
		...
		const en = {
			"title": "Blue Whale",
			"hint": "Sidebar whale, hero whale, tab favicon, and brand tokens use DeepSeek Chat blue #4D6BFE. Light and dark follow the built-in appearance.",
			"open": "Turn on",
			"close": "Turn off",
			"statusOn": "On"
		};
					...
					(0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							(0, react_jsx_runtime.jsx)("button", {
								type: "button",
								className: enabled ? "dsh-blue-whale-btn dsh-blue-whale-btn-close" : "dsh-blue-whale-btn dsh-blue-whale-btn-open",
